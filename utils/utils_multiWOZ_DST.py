...
EXPERIMENT_DOMAINS = ["hotel", "train", "restaurant", "attraction", "taxi","Banks_1","Buses_1","Buses_2","Calendar_1","Events_1"\
                        ,"Events_2","Flights_1","Flights_2","Homes_1","Hotels_1","Hotels_2","Hotels_3","Media_1","Movies_1","Music_1"\
                        ,"Music_2","RentalCars_1","RentalCars_2","Restaurants_1","RideSharing_1","RideSharing_2","Services_1","Services_2","Services_3"\
                        ,"Travel_1","Weather_1","Alarm_1","Banks_2","Flights_3","Hotels_4","Media_2","Movies_2","Restaurants_2","Services_4"\
                        ,"Buses_3","Events_3","Flights_4","Homes_2","Media_3","Messaging_1","Movies_3","Music_3","Payment_1","RentalCars_3"\
                        ,"Trains_1","bus","police","hospital"]
...
class Lang:
    ...
    def index_words(self, sent, type,domain=None):
        if type == 'utter':
            ...
            for slot in sent:
                pattern = r'(\w+)-(.+)'
                d=re.search(pattern, slot).group(1)
                s=re.search(pattern, slot).group(2)
                if "_" in d:
                    for dd in d.split("_"):
                        self.index_word(dd)
                if "_" not in d:
...
class Dataset(data.Dataset):
    ...
    def preprocess_domain(self, turn_domain):
        domains = {"hotel":0, "train":1, "restaurant":2, "attraction":3, "taxi":4,"Banks_1":5,"Buses_1":6,"Buses_2":7,"Calendar_1":8,"Events_1":9\
                        ,"Events_2":10,"Flights_1":11,"Flights_2":12,"Homes_1":13,"Hotels_1":14,"Hotels_2":15,"Hotels_3":16,"Media_1":17,"Movies_1":18,"Music_1":19\
                        ,"Music_2":20,"RentalCars_1":21,"RentalCars_2":22,"Restaurants_1":23,"RideSharing_1":24,"RideSharing_2":25,"Services_1":26,"Services_2":27,"Services_3":28\
                        ,"Travel_1":29,"Weather_1":30,"Alarm_1":31,"Banks_2":32,"Flights_3":33,"Hotels_4":34,"Media_2":35,"Movies_2":36,"Restaurants_2":37,"Services_4":38\
                        ,"Buses_3":39,"Events_3":40,"Flights_4":41,"Homes_2":42,"Media_3":43,"Messaging_1":43,"Movies_3":44,"Music_3":45,"Payment_1":46,"RentalCars_3":47\
                        ,"Trains_1":48,"bus":49,"police":50,"hospital":51}
...
def read_langs(file_name, gating_dict, SLOTS, dataset, lang, mem_lang, sequicity, training, max_line = None):
    ...
    with open(file_name) as f:
        ...
        for dial_dict in dials:
            ...
            for domain in dial_dict["services"]:
            ...
            for ti, turn in enumerate(dial_dict["turns"]):
                if ti%2==1:
                    turn_uttr=turn["utterance"]+ " ; "
                if ti %2==0:
                    turn_id = math.ceil(ti/2)
                    turn_uttr=turn_uttr+turn["utterance"]+ " ; "
                    turn_uttr_strip = turn_uttr.strip()
                    dialog_history += turn_uttr
                    source_text = dialog_history.strip()
                    if (len(turn['frames'])!=0):
                        turn_domain=turn['frames'][0]['service']
                        turn_belief_dict = fix_general_label_error(turn["frames"][0]["state"]["slot_values"], False, SLOTS,turn_domain)
                    
                    if (len(turn['frames'])==0):
                        for i in dial_dict["turns"]:
                            if i['frames']!=[]:
                                if i['frames'][0]['service']!=[]:
                                    turn_domain=i['frames'][0]['service']
                                    break
                        turn_belief_dict={}
                    ...
                    if (args["all_vocab"] or dataset=="train") and training:
                        mem_lang.index_words(turn_belief_dict, 'belief',turn_domain)

                    class_label, generate_y, slot_mask, gating_label  = [], [], [], []
                    start_ptr_label, end_ptr_label = [], []
                    #print('turn_belief_dic',turn_belief_dict.keys())
                    ...
                    if len(dial_dict["services"])==0:
                        for i in dial_dict["turns"]:
                            if i['frames']!=[]:
                                if  i['frames'][0]['service']!=[]:
                                    services=i['frames'][0]['service']
                                    break
                    else:
...
def read_test_langs(file_name, gating_dict, SLOTS, dataset, lang, mem_lang, sequicity, training, max_line = None):
    print(("Reading from {}".format(file_name)))
    data = []
    max_resp_len, max_value_len = 0, 0
    domain_counter = {} 
...
def get_test_seq(pairs, lang, mem_lang, batch_size, type, sequicity):  
    if(type and args['fisher_sample']>0):
        shuffle(pairs)
        pairs = pairs[:args['fisher_sample']]
    ...
    dataset = TestDataset(data_info, lang.word2index, lang.word2index, sequicity, mem_lang.word2index)
...
def prepare_data_seq(training, task="dst", sequicity=0, batch_size=100):
    ...
    file_train = 'data2/train_dials2.json'
    file_dev = 'data2/dev_dials2.json'
    file_test = 'data2/test_dials.json'
    ...
    ontology = json.load(open("data2/ontology.json", 'r'))
    ...
    if training:
        ...
        pair_test, test_max_len, slot_test = read_test_langs(file_test, gating_dict, ALL_SLOTS, "test", lang, mem_lang, sequicity, training)
        test  = get_test_seq(pair_test, lang, mem_lang, eval_batch, False, sequicity)
        if os.path.exists(folder_name+lang_name) and os.path.exists(folder_name+mem_lang_name):
            print("[Info] Loading saved lang files...")
            print(folder_name+lang_name)
        ...
        emb_dump_path = 'data2/emb{}.json'.format(len(lang.index2word))
    ...
    test_4d = []
    if args['except_domain']!="":
        pair_test_4d, _, _ = read_test_langs(file_test, gating_dict, ALL_SLOTS, "dev", lang, mem_lang, sequicity, training)
